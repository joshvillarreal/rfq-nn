...
function initscoresdict(n_folds; by_response=false)
    ...
    if !by_response
        ...
        scores_dict["training_times"] = Vector{Float32}(undef, n_folds)
    else
        scores_dict["mape_train"] = Vector{Float32}(undef, n_folds)
        scores_dict["mape_val"] = Vector{Float32}(undef, n_folds)
    end
