...
def get_a_from_m_r0(m, k, r0):
    """
    :param m: modulation
    :param k: wave number
    :param r0: aperture in center of cell where rx = ry
    :return: aperture
    """

    def func(_a):
        _aa = (m**2.0 - 1.0) / (
            m**2.0 * bessel1(0.0, k * _a) + bessel1(0.0, k * m * _a)
        )
        _xx = 1.0 - _aa * bessel1(0.0, k * _a)
        _r0 = _a / np.sqrt(_xx)

        return _r0 - r0

    return root(func, np.array([1e-3])).x[
        0
    ]  # 2.0/1e-3 is a good starting point for modulation/ap search


def compute_cellnumber_2(row):
    """
    row: Dict-like
